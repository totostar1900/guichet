...
- 2026-09-22 evening: seven views in IndexChart (niveau, volumes, contributions, calendrier, societes, capitalisation, flottant), data panel, CSV export, full sessions table; per-session caps = last share count × close; float reading is chained float-weighted returns, labelled non-published;  assembles everything.
