...
metadata: 
  ...
  modified: 2026-09-19T01:35:02.304Z
---
...
**How to apply:** write a colon for a lead or an explanation ("Comment lire : …", "Solidité : fonds propres…"), commas or parentheses for an aside, a middle dot (·) for structural separators in names and labels (SVT names, "VL · dépositaire"). The lone "—" that stands for a missing value in a table cell stays (it is a symbol, not punctuation), and the glossary's mention of it («, ») too. Never split code on ", " again: company comments split on " : " (short lead only), the search's definition titles on " : ", SVT names on " · ". The sweep script is scratchpad/dedash.mjs (one spaced dash → colon, two or more → commas, "," → ","). See [[guichet-project]].
