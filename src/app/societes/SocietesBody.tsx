...
/**
 * Les sociétés cotées, une seule fois.
 *
 * Le Guichet les montre au client, le desk les lit chez lui : ce sont les
 * mêmes sociétés, lues au même bulletin. C'est « DeskView », autour de ce
 * corps, qui dit où mènent les rangées.
 */
export async function SocietesBody({ mode = "client" }: { mode?: "client" | "desk" } = {}) {
  const t = await getT();
  const r = repo();
