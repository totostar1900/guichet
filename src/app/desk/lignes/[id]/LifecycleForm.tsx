...
export function LifecycleForm({ offerId, current, withdrawn }: { offerId: string; current: number; withdrawn: boolean }) {
  ...
  return (
    <form action={action} className={styles.restore}>
      ...
      <input name="reason" placeholder={withdrawn ? "Pourquoi la remettre en ligne ?" : "Pourquoi la retirer ? (erreur, annulation, demande de l'émetteur…)"} aria-label="Motif" required minLength={3} maxLength={300} />
