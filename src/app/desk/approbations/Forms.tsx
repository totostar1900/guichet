...
export function DecideForm({ id }: { id: string }) {
  ...
  return (
    <form action={action} className={styles.decide}>
      ...
      <input name="note" placeholder="Note (obligatoire pour refuser)" aria-label="Note" maxLength={500} />
  ...
  return (
    <form action={action} className={styles.policy}>
      <label className={styles.check}>
        <input type="checkbox" name="enabled" defaultChecked={p.enabled} /> Fenêtre déléguée active (hors fenêtre, un responsable approuve)
      </label>
      <div className={styles.grid}>
        <label>
          <span>Prix OTA / APE — minimum (%)</span>
          <input name="priceMin" type="number" step="0.5" defaultValue={p.pricePct.min} />
        </label>
        <label>
          <span>Prix OTA / APE — maximum (%)</span>
          <input name="priceMax" type="number" step="0.5" defaultValue={p.pricePct.max} />
        </label>
        <label>
          <span>Taux précompté BTA — minimum (%)</span>
          <input name="rateMin" type="number" step="0.05" defaultValue={p.precountRate.min} />
        </label>
        <label>
          <span>Taux précompté BTA — maximum (%)</span>
          <input name="rateMax" type="number" step="0.05" defaultValue={p.precountRate.max} />
        </label>
        <label>
          <span>Cours saisi : écart maximal vs dernier cours (%)</span>
