...
export function Convention({ number, file, now }: { number: string; file?: ClientFile; now: Date }) {
  ...
  const articles: [string, string][] = [
    ...
    ["6. Tarifs", "Commission d'intermédiation par opération selon l'annexe tarifaire en vigueur, déduite du rendement brut ; droits de garde annuels ; aucun frais d'ouverture. Toute modification tarifaire est notifiée trente jours avant application."],
