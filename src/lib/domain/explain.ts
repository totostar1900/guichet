...
export function explainKpis(o: Offer, now = new Date()): KpiExplanation[] {
  ...
  if (o.kind === "FONDS" && o.fund) {
    const f = o.fund;
    return [
      { key: "perf", title: f.perf1yPct != null ? "Performance sur 12 mois" : "Depuis l'origine", term: "vl", lines: [["VL il y a 12 mois → aujourd'hui", f.perf1yPct != null ? signed(f.perf1yPct) : "—"], ["Depuis l'origine", signed(f.perfSinceInceptionPct)], ["Dernière VL", `${fmt(f.nav)} FCFA le ${fmtDate(f.navDate)}`]], caveats: ["Performance passée, nette des frais de gestion prélevés dans la VL ; ne préjuge pas de l'avenir.", f.entryFeePct ? `Frais d'entrée du fonds : ${fmtPct(f.entryFeePct, 2)}.` : "Sans frais d'entrée."] },
