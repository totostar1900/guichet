  ...
  if (o.kind === "MARCHE") {
    ...
    if (bond) {
      ...
      return [
        ...
        { key: "last", title: "Dernier cours", term: "cours", lines: [["Dernier cours", o.lastPrice != null ? `${fmtPrice(o.lastPrice)} du nominal` : "—"], ["Séance", o.lastPriceOn ? fmtDate(o.lastPriceOn) : "—"], ["Dernier échange", o.lastTradedOn ? fmtDate(o.lastTradedOn) : "aucun relevé"], ["Acheteur / vendeur", `${o.bid != null ? fmtPrice(o.bid) : "—"} / ${o.ask != null ? fmtPrice(o.ask) : "—"}`]], caveats: ["Cours de clôture du Bulletin Officiel de la Cote, repris sans retraitement : le bulletin cote la ligne à chaque séance, qu'elle ait traité ou non.", "Un marché étroit : un ordre peut rester non exécuté plusieurs séances."] },
      ];
    }
