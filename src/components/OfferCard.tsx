...
/**
 * One number, three facts, one action. Everything else is on the fiche, or in
 * the « ··· ». Compact (the phone's choice): two lines, the title, then the
 * number with its condition and the date.
 */
    ...
    <article className={`${styles.card} ${s.past ? styles.past : ""} ${compact ? styles.compact : ""}`} style={{ ["--card-c" as string]: `var(--fam-${s.family}, ${famVars(s.family)["--fam-c"] ?? "var(--line-2)"})` }}>
      ...
      <div className={styles.row2}>
        ...
        <div className={styles.act}>
          ...
          <span className={styles.when}>{s.deadline === "continue" ? t("cotation continue") : t(s.deadline)}</span>
        </div>
      </div>
