...
/** The document chain of one order. */
function Documents({ lang }: { lang: Lang }) {
  const steps: [string, string][] = [
    [T(lang, "Accusé de réception", "Acknowledgement"), T(lang, "à l'intention", "at the intention")],
    [T(lang, "Bulletin d'ordre", "Order form"), T(lang, "à la confirmation", "at confirmation")],
    [T(lang, "Appel de fonds", "Call for funds"), T(lang, "à la confirmation", "at confirmation")],
    [T(lang, "Bordereau", "Submission slip"), T(lang, "à la transmission", "at transmission")],
    [T(lang, "Avis de résultat", "Result notice"), T(lang, "aux résultats", "at results")],
    [T(lang, "Avis de coupon · Relevé", "Coupon notice · Statement"), T(lang, "pendant la vie du titre", "during the security's life")],
  ];
  return (
    <svg viewBox="0 0 760 150" role="img" aria-label={T(lang, "La chaîne documentaire", "The document chain")}>
      ...
      {steps.map(([name, when], i) => {
        const x = 8 + i * 125;
        return [box(x, 24, 116, 64, `b${i}`), ...wrap(name, 18).map((line, k) => label(x + 58, 46 + k * 13, line, `n${i}${k}`, { size: 11, weight: 800 })), label(x + 58, 78, when, `w${i}`, { size: 9.5, weight: 500, fill: "var(--ink-3)" }), i < steps.length - 1 ? arrow(x + 116, 56, x + 125, 56, `a${i}`) : null];
      })}
