...
export function FlowsChart({ r, settleOn }: { r: BondResult; settleOn: string }) {
  ...
  const padL = 16;
  const padR = 16;
  const base = 128; // the zero line
  const upRoom = 92; // above the axis: bars + value label
  const downRoom = 56; // below: the outlay bar, its label sits under it
  ...
  return (
    <div style={{ position: "relative" }}>
      <svg className="chart" viewBox={`0 0 ${W} ${H}`} role="img" aria-label="Flux de trésorerie" onMouseLeave={() => setHover(null)}>
        ...
        {pts.map((p, i) => {
          ...
          return (
            <g key={i} onMouseEnter={() => setHover(i)} onTouchStart={() => setHover(i)} style={{ opacity: hover != null && hover !== i ? 0.55 : 1 }}>
              ...
              <text x={x + bw / 2} y={H - 22} textAnchor="middle" style={{ fontSize: dense ? 9 : undefined }}>
