  ...
  chapters: [
    ...
    },
    {
      ...
      blocks: [
        ...
        { type: "p", text: l("Les situations qui reviennent ont leur message préparé : sur un ordre, « Écrire au client » propose le mot qui convient à son état (pièces manquantes, compte à ouvrir, provision attendue, échéance, relance, appel), le remplit avec l'ordre, et vous le laissez corriger avant de l'ouvrir dans WhatsApp ou dans votre courrier. Rien ne part de l'application : c'est votre main qui envoie, et le fil de l'ordre garde la trace du message ouvert. Les textes eux-mêmes se relisent dans Référentiel › Modèles › Messages préparés, comme les passages d'un document.", "The situations that come round have their prepared message: on an order, “Write to the client” offers the word its state calls for (missing documents, account to open, funds awaited, deadline, follow-up, call), fills it in with the order, and lets you correct it before opening it in WhatsApp or in your mail. Nothing leaves the application: your own hand sends it, and the order's thread keeps the trace of the message opened. The wordings themselves are reviewed in Reference data › Models › Prepared messages, like a document's passages.") },
        { type: "note", kind: "rule", text: l("Un message préparé qui demande quelque chose exige la précision écrite : « il nous manque des pièces » sans dire lesquelles fait perdre le tour suivant. L'écran refuse d'ouvrir le message tant qu'un champ reste à remplir.", "A prepared message that asks for something requires the written detail: “we are missing documents” without saying which loses the next round. The screen refuses to open the message while a field is left to fill.") },
