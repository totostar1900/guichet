      ...
      blocks: [
        ...
        },
        {
          ...
          items: [
            ...
            l("Leçons : « Dupliquer » sur une ligne ouvre une nouvelle leçon remplie comme l'originale (clé suffixée, ordre suivant, titre « (copie) ») ; changez ce qui doit l'être et « Créer cette copie ». Une clé déjà prise est refusée ; l'originale ne bouge pas.", "Lessons: “Duplicate” on a row opens a new lesson filled like the original (suffixed key, next order, title “(copie)”); change what must change and “Create this copy”. A key already taken is refused; the original does not move."),
            l("Sociétés cotées et émetteurs : la fiche se remplit dans un formulaire (identité, capital et actionnaires, chiffres par exercice, documents, lecture), avec des lignes qu'on ajoute et retire. Une société ou un émetteur nouveau n'est pas créé à la main : sa ligne arrive avec le bulletin, et l'onglet propose « Créer la fiche » (ce que le bulletin sait est pré-rempli) ; « Dupliquer » part d'une fiche voisine.", "Listed companies and issuers: the fiche is filled in a form (identity, capital and shareholders, figures per year, documents, reading), with rows to add and remove. A new company or issuer is not created by hand: its line arrives with the bulletin, and the tab offers “Create the fiche” (what the bulletin knows is pre-filled); “Duplicate” starts from a neighbouring fiche."),
            l("Échéanciers : une ligne par obligation cotée. En orange, celles sans échéancier exact (le prix court sur l'année du bulletin) avec « Créer l'échéancier » pré-rempli ; en gris, les échéanciers de lignes échues ou retirées (« hors bulletin »). La boîte filtre par ISIN, émetteur, ligne ou source, le menu par état, les colonnes se trient.", "Schedules: one row per listed bond. In orange, those without an exact schedule (the price runs on the bulletin's year) with “Create the schedule” pre-filled; in grey, schedules of matured or delisted lines (“off the bulletin”). The box filters by ISIN, issuer, line or source, the menu by state, columns sort."),
          ],
        },
