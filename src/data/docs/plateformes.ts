      ...
      blocks: [
        ...
        { type: "lead", text: l("Deux abonnements payants suffisent pour tourner en production : Vercel Pro et Supabase Pro. Le reste est gratuit. Les prix sont les tarifs publics connus au 18 septembre 2026, à vérifier avant de budgéter.", "Two paid plans are enough to run in production: Vercel Pro and Supabase Pro. Everything else is free. Prices are the public list prices known on 18 September 2026; check before budgeting.") },
        {
          ...
          rows: [
            ...
            [l("Vercel (projet purpose-capital/guichet)", "Vercel (project purpose-capital/guichet)"), l("Héberge l'application, exécute les neuf tâches planifiées, garde toutes les clés (variables d'environnement).", "Hosts the app, runs the nine scheduled tasks, holds every secret (environment variables)."), l("Propriétaire de l'équipe : Georges ; membres ajoutés dans Vercel › Team.", "Team owner: Georges; members added in Vercel › Team."), l("Pro obligatoire : le plan Hobby est réservé à l'usage non commercial, limité à deux tâches quotidiennes et à des fonctions courtes. 20 $ par membre et par mois.", "Pro required: the Hobby plan is non-commercial only, limited to two daily tasks and short functions. $20 per member per month.")],
            ...
            [l("Cloudflare (facultatif, pas encore monté)", "Cloudflare (optional, not set up yet)"), l("Un seul rôle : Email Routing et un Worker transmettraient intake@ à l'application. Le DNS du domaine n'est pas ici.", "One role only: Email Routing and a Worker would forward intake@ to the app. The domain's DNS is not here."), l("Compte : Georges.", "Account: Georges."), l("Gratuit (paliers gratuits)", "Free (free tiers)")],
            [l("Nom de domaine et zone DNS", "Domain name and DNS zone"), l("Adresse de l'application, domaine d'envoi des e-mails, numéro WhatsApp rattaché. Les deux CNAME vers Vercel se posent dans la zone.", "App address, e-mail sending domain, WhatsApp business number. The two CNAMEs to Vercel are set in the zone."), l("Nom déposé chez GoDaddy ; zone servie par Netlify DNS, c'est là que se posent les enregistrements. Compte : Georges.", "Name registered at GoDaddy; zone served by Netlify DNS, which is where records are set. Account: Georges."), l("10 à 40 $ par an selon l'extension", "$10 to $40 a year depending on the TLD")],
      ...
      blocks: [
        { type: "lead", text: l("Guichet n'a pas de mots de passe : on se connecte avec un code à usage unique reçu par e-mail, et l'équipe ajoute un second facteur. Il n'y a donc pas de « mot de passe oublié » : perdre l'accès, c'est redemander un code.", "Guichet has no passwords: you sign in with a one-time code received by e-mail, and the team adds a second factor. So there is no \"forgot password\": losing access just means asking for a new code.") },
        {
          ...
          rows: [
            ...
            [l("Code par téléphone (facultatif, désactivé)", "Phone code (optional, off)"), l("Supabase Auth › Phone avec un fournisseur (Twilio SMS ou canal WhatsApp Twilio) ; variable PHONE_OTP_ENABLED. Le même code sert à la connexion et à la preuve du numéro dans Mon espace › Sécurité.", "Supabase Auth › Phone with a provider (Twilio SMS or Twilio WhatsApp); PHONE_OTP_ENABLED variable. The same code serves the sign-in and the number proof in My space › Security."), l("Compte Twilio ; payé au message (environ 0,05 à 0,10 $ par SMS vers le Cameroun, à vérifier).", "Twilio account; paid per message (about $0.05 to $0.10 per SMS to Cameroon, to verify).")],
