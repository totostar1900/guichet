...
- **Guide** (`/desk/guide`) : chaque page du desk champ par champ, des captures (`npm run guide:shots` sur `npm run dev:memory`, fichiers `public/guide/*.png`, une galerie possible par section : Documentation montre l'index, les six pages, l'Aide côté desk et côté client, le chapitre « Comment Guichet est entretenu », la page Guide (/info), l’écran des premiers pas et les notes de travail ; Référentiel montre le Guide, l’Aide et ce chapitre) et la visite guidée (20 étapes, `TOUR` dans `src/data/desk-guide.ts` ; une étape peut porter un lien et un aperçu). Côté client : « Premiers pas » (six écrans, rejouable depuis le Guide), et des repères sur la fiche, le Guide, Actualités et Aide.
